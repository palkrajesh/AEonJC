...
public class AcornCore {
    ...
    public AcornCore(){
        ...
        tempbuf = JCSystem.makeTransientByteArray((short)293, JCSystem.CLEAR_ON_DESELECT);
    ...
    public byte Encrypt_StateUpdate128(byte ca, byte cb) {
        byte f;
        //ISOException.throwIt((short)0x0008);  // 292 ms (since start)
        ...
        //ISOException.throwIt((short)0x0009);  // 4 ms
        f = FBK128(ca, cb);
        
        //ISOException.throwIt((short)0x0010);  // 5 ms
        //for(short j = 0; j <= 291; j++) state[j] = state[(short)(j+1)];
        Util.arrayCopyNonAtomic(state, (short)1, tempbuf, (short)0, (short)292);
        Util.arrayCopyNonAtomic(tempbuf, (short)0, state, (short)0, (short)292);
        state[(short)292] = (byte)(f ^ plaintextbit);
        ciphertextbit = (byte)(ks ^ plaintextbit);
        
        //ISOException.throwIt((short)0x0011);    // 126 ms (before); 3 ms (after optimisation)
        return SUCCESS;
    }
    ...
    public byte KSG128() {
        return (byte)(state[(short)12] ^ state[(short)154] ^ maj(state[(short)235], state[(short)61], state[(short)193]) );
    }
    ...
    public byte acorn128_enc_onebyte(byte cabyte, byte cbbyte) {
        //ISOException.throwIt((short)0x0008); // 41082 ms (since start)
        byte caBit, cbBit, i;
        ciphertextbyte = (byte)0x00;
        ksbyte = 0;
        ks = 0;
        for(i = 0; i < 8; i++) {
            //ISOException.throwIt((short)0x0009); //
            caBit = (byte)( (byte)(cabyte >> i) & (byte)1);
            cbBit = (byte)( (byte)(cbbyte >> i) & (byte)1);
            plaintextbit = (byte)( (byte)(plaintextbyte >> i) & (byte)1);
            //ISOException.throwIt((short)0x0010); // 69 ms
            Encrypt_StateUpdate128(caBit, cbBit);
            //ISOException.throwIt((short)0x0011); // 34 ms
            ciphertextbyte |= (byte)(ciphertextbit << i);
            ksbyte |= (byte)(ks << i);
            //ISOException.throwIt((short)0x0012); // 80 ms in function
        }
        ISOException.throwIt((short)0x0013); // 377 ms (since start)
    ...
    public byte acorn128_tag_generation() {
        ...
        for(i = 0; i < (byte)(512/8); i++) {
            ...
            if ( i >= (byte)((byte)(512/8) - 16) ) {mac[(byte)(i-(byte)((byte)(512/8)-16))] = ksbyte; }
        }
        return SUCCESS;
    }
    ...
    public byte crypto_aead_encrypt() {
        ...
        //ISOException.throwIt((short)0x0001);  // 158 ms (since start); 105 ms (after optimisation)
        
        //initialization stage
        acorn128_initialization();
        
        //ISOException.throwIt((short)0x0002);  // 233582 ms (because of one bit encryption for 1536 times); // 41266 ms (after optimisation)
        
        //process the associated data
        for(i = 0; i < adlen; i++) {
            plaintextbyte = ad[i];
            //ISOException.throwIt((short)0x0003);  // 346 ms
            acorn128_enc_onebyte((byte)0xff, (byte)0xff);
            //ISOException.throwIt((short)0x0004);  // 1363 ms; 343 (after optimisation)
        }
        ...
        //ISOException.throwIt((short)0x0005);  // 112061 ms (because of 80 byte encryption); 31752 ms (after optimisation)
        ...
        //ISOException.throwIt((short)0x0006);  // 282156 ms; 86106 ms (after optimisation)
        ...
        //ISOException.throwIt((short)0x0007);  // 90564 ms; 184880 ms (after optimisation)
